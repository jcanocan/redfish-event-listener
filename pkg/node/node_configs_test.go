...
// minimal negotiated serializer that uses the unstructured JSON scheme
type unstructuredNegotiatedSerializer struct {
	serializer runtime.Serializer
}

func (n unstructuredNegotiatedSerializer) SupportedMediaTypes() []runtime.SerializerInfo {
	return []runtime.SerializerInfo{
		{
			MediaType:        runtime.ContentTypeJSON,
			Serializer:       n.serializer,
			PrettySerializer: n.serializer,
			StrictSerializer: n.serializer,
		},
	}
}
func (n unstructuredNegotiatedSerializer) EncoderForVersion(e runtime.Encoder, _ runtime.GroupVersioner) runtime.Encoder {
	return e
}
func (n unstructuredNegotiatedSerializer) DecoderToVersion(d runtime.Decoder, _ runtime.GroupVersioner) runtime.Decoder {
	return d
}

func newDynamicClientWithListResponse(list *unstructured.UnstructuredList, gv schema.GroupVersion) *dynamic.DynamicClient {
	data, err := json.Marshal(list)
	if err != nil {
		panic(err)
	}
	rc := &restfake.RESTClient{
		NegotiatedSerializer: unstructuredNegotiatedSerializer{serializer: unstructured.UnstructuredJSONScheme},
		GroupVersion:         gv,
...
var _ = Describe("Node configs", func() {
	Context("from FenceAgentsRemediationTemplate", func() {
		...
		It("should return node configs for valid fence_ipmilan entries in the namespace", func() {
			...
			dyn := newDynamicClientWithListResponse(list, gv)
		...
		It("returns error when required maps are missing", func() {
			...
			Expect(err.Error()).To(ContainSubstring("failed to find '--username' parameter"))
		})
	})
})
